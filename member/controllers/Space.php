...
class Space extends M_Controller {
		...
		if (IS_POST) {
			...
			if (isset($post['error'])) {
				...
				if ($error == 0) {
					...
					// 操作成功
					$this->member_msg(lang('000'), man_url('space/index'), 1);
				} else {
					// 操作成功,等待审核
					$this->member_msg(lang('m-240'), man_url('space/index'), 2);
				}
			}
		} else {
			$data = $this->space;
	...
	public function template() {
		...
		if ($style && $this->space['style'] != $style) {
			$rule = man_string2array(@file_get_contents(FCPATH.'member/templates/'.$style.'/rule.php'));
			if ($style == 'default' || isset($rule[$this->markrule])) {
				$this->db
					 ->where('uid', (int)$this->uid)
					 ->update($this->db->dbprefix('space'), array('style' => $style));
				$this->member_msg(lang('m-319'), man_member_url('space/template'), 1);
			} else {
		...
		$list = array();
		$data = array_diff(man_dir_map(FCPATH.'member/templates/', 1), array('admin', 'member'));
		if ($data) {
			foreach ($data as $dir) {
				$tpl = array(
					'name' => $dir,
				...
				$rule = man_string2array(@file_get_contents(FCPATH.'member/templates/'.$dir.'/rule.php'));
				if ($dir == 'default') {
					$list[] = $tpl;
