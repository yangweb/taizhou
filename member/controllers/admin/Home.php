...
class Home extends M_Controller {
    ...
    public function index() {
		...
		if (IS_POST && $this->input->post('action')) {
			...
			if ($this->input->post('action') == 'del') {
				...
				$note = man_lang('m-078', $this->member['username'], $this->get_cache('member', 'group', $gid, 'name'));
				$this->db
					 ->where_in('uid', $ids)
		...
		$this->template->assign(array(
            ...
            'field' => $field,
			'param'	=> $search,
			'pages'	=> $this->get_pagination(man_url('member/home/index', $param), $param['total']),
		));
    ...
    public function add() {
		...
		if (IS_POST) {
			...
			if (!$data['groupid']) {
                exit(man_json(0, lang('m-156'), 'groupid'));
            }
			
			if ($all) {
				// 批量添加
				if (!$info) {
                    exit(man_json(0, lang('m-155'), 'info'));
                }
				...
				exit(man_json(1, man_lang('m-157', $success, $error)));
			} else {
				// 单个添加
				$uid = $this->member_model->register(array(
		...
		if (IS_POST) {
			...
			if (!$edit['groupid']) {
				...
				if ($edit['password']) {
					...
					$this->member_model->add_notice($uid, 1, man_lang('m-079', $this->member['username']));
				}
                // 修改邮箱
                if ($edit['email'] != $data['email']) {
                    ...
                    $this->member_model->add_notice($uid, 1, man_lang('m-121', $this->member['username']));
                }
				...
				$this->admin_msg(lang('000'), man_url('member/home/edit', array('uid' => $uid, 'page' => $page)), 1);
			}
			$this->admin_msg($error, man_url('member/home/edit', array('uid' => $uid, 'page' => $page)));
		}
